...
def parse(datafile, N):
	adj_list = [[] for i in range(N+5000)]
	with open(datafile, 'r') as f:
		for line in f:
			line_new = line.split(': ')
			child = map(int, line_new[1].split())
			adj_list[int(line_new[0])] = child
	return adj_list
...
def find_maxdeg(g):
	...
	deglist_sorted = dict(sorted(deglist.items(), key=lambda item: item[1], reverse=True))  # sort in descending order of node degree
	# print(deglist_sorted)
	for key in deglist_sorted:
		v = (key, deglist_sorted[key])  # tuple - (node,degree)
		break
	return v
...
def main(inputfile, cutoff, N):
	...
	print('\nHasil vertex cover:')
	print(','.join([str(x[0]) for x in Sol_VC]))
